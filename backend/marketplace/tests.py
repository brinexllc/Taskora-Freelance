...
class AuthenticationTests(BaseTests):
    def setUp(self):
        super().setUp()
        self.form = dict(first_name='Aziz', last_name='Rahimov', username='aziz_01', birth_date='2000-06-01', phone='+998901234567', has_passport=True, email='aziz@example.com', password='Secure-2026-pass', password_confirm='Secure-2026-pass')
    def test_registration_requires_all_spec_fields(self):
        for field in ['first_name','last_name','username','birth_date','phone','email','has_passport','password','password_confirm']:
    ...
    def test_register_choose_role_three_login_identifiers_logout(self):
        ...
        self.as_user(user)
        self.assertEqual(self.post('projects', dict(title='Test',description='Work',budget_min=1000,budget_max=2000)).status_code, 403)
    ...
    def test_rejects_invalid_details_and_duplicate_identity(self):
        for changes in [{'phone':'+79012345678'},{'phone':'+9981234567890'},{'has_passport':False},{'username':'two users'},{'password':'weakpass','password_confirm':'weakpass'},{'password':'lowercase1!','password_confirm':'lowercase1!'},{'password':'NoDigitHere!','password_confirm':'NoDigitHere!'},{'password':'NoSymbol123','password_confirm':'NoSymbol123'}]:
...
class MarketplaceTests(BaseTests):
    def setUp(self):
        ...
        self.project=Project.objects.create(owner=self.customer,title='IT project',description='Build a dashboard',budget_min=10000,budget_max=50000,client_name='Customer',skills=['React'])
    ...
    def work(self):
        ...
        response=self.client.post(f'/api/contracts/{contract}/submit/',{'file':SimpleUploadedFile('project.zip',b'private original bytes'),'preview_text':'Dashboard implemented'},format='multipart')
        self.assertEqual(response.status_code,200,response.data)
        return contract
    def test_public_directory_and_ownership(self):
        ...
        self.assertEqual(self.post('projects',{'title':'Test','description':'Test','budget_min':1000,'budget_max':2000}).status_code,403)
    ...
    def test_review_revisions_wallet_payment_download_archive(self):
        ...
        self.as_user(self.freelancer)
        response=self.client.post(f'/api/contracts/{contract}/submit/',{'file':SimpleUploadedFile('final.zip',b'final bytes'),'preview_text':'Tests added'},format='multipart')
        self.assertEqual(response.status_code,200)
        self.as_user(self.customer)
        self.assertEqual(self.post('payments/checkout',{'contract':contract,'provider':'wallet'}).status_code,400)
        Profile.objects.filter(user=self.customer).update(balance=Decimal('50000'))
        response=self.post('payments/checkout',{'contract':contract,'provider':'wallet','amount':'1'})
        self.assertEqual(response.status_code,200,response.data)
        ...
        self.assertEqual(b''.join(result.streaming_content),b'final bytes')
    ...
    def test_profile_settings_and_unverified_skills(self):
        ...
        self.assertEqual(response.status_code,200,response.data)
        self.assertEqual(response.data['profile']['skills'],['Python','Django'])
    ...
    def test_withdrawal_reservation_cancel_and_processing(self):
        ...
        response=self.post('wallet/withdraw',{'amount':'4000','destination':'Bank •••• 1234'})
        self.assertEqual(response.status_code,201,response.data)
...
class ClickTests(MarketplaceTests):
    ...
    def test_click_signature_amount_prepare_complete_replay(self):
        contract=self.work(); self.as_user(self.customer)
        result=self.post('payments/checkout',{'contract':contract})
        self.assertEqual(result.status_code,200,result.data)
        ...
        self.assertEqual(Profile.objects.get(user=self.freelancer).balance,25000)
        self.assertEqual(WalletEntry.objects.filter(kind='income').count(),1)
        self.assertEqual(Contract.objects.get(pk=contract).status,'completed')
